...
%% @return returns {ok, GameServerPid} or if not found returns {error, game_not_found, GameId}
get_game(GameId) ->
...
%% @return reply {reply, {ok, Pid}, S} if game found or {reply, {error, game_not_found, GameId}, S} if game not found
