...
%% @doc bind a session server to a given websocket handler ( @see grdl_ws_handler )
...
%% @doc asynchronously handle a new message from a websocket handler ( @see grdl_ws_handler )
...
%% @doc pass a websocket message along to the given websocket handler ( @see grdl_ws_handler )
%% Because other processes ( @see grdl_game_serv ) do not have direct access to websocket
...
%% @param Msg the JSX map-format message to be forwarded to the @see grdl_ws_handler
...
%% @doc bind to a websocket @see bind_to_ws/2
...
%% @doc respond to a websocket sess_echo message @see handle_message/2
...
%% @doc handle a websocket set_username message @see handle_message/2
...
%% @doc handle a websocket message to create a new game @see handle_message/2
%% Takes message with join_game atom mapped to binary <<"new">> along
%% with undefined g_pid in state. Spawns a game ( @see grdl_game_pool_serv ),
...
%% @doc handle a websocket message to join an existing game @see handle_message/2
%% Takes message with join_game atom mapped to binary along
%% with undefined g_pid in state. Searches for a game ( @see grdl_game_pool_serv ),
...
%% @doc handle a websocket message to start the bound game @see handle_message/2
%% Takes message with start_game atom mapped to anything, along with defined g_pid
%% in state. Starts the game ( @see grdl_game_serv:start_game/1 ). State unchanged.
handle_cast({ws_message, _ = #{start_game := _}}, S = #state{g_pid = Pid}) ->
  grdl_game_serv:start_game(Pid),
  {noreply, S};

%% @doc handle a websocket message to submit a guess to the bound game @see handle_message/2
%% Takes message with guess atom mapped to anything, along with defined g_pid
%% in state. Submits the guess ( @see grdl_game_serv:submit_guess/2 ). State unchanged.
handle_cast({ws_message, _ = #{guess := Guess}}, S = #state{g_pid = Pid}) ->
  grdl_game_serv:submit_guess(Pid, Guess),
  {noreply, S};

%% @doc forward a websocket message to the bound websocket handler @see send_message/2
%% Takes message, along with defined ws_pid in state. Sends the message through the websocket
%% ( @see grdl_ws_handler:websocket_send/2 ). State unchanged.
